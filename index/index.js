$(document).ready(function() {
    ...
    var onPageLoad = {
        ...
        getMakes: function() {
            $.get(enmundsBaseUrl + "makes?state=used&year=2015&view=basic&fmt=json&api_key=" + edmundsApiKey, function(data) {
                console.log(data);
                data.makes.splice(1, 1);
                data.makes.splice(37, 1);
                data.makes.splice(29, 1);
                data.makes.splice(20, 1);
                var makes = Handlebars.compile($('#carMakes').html());
                var makeData = makes(data.makes);
                $('.makeConatiner').append(makeData);
            });
        }
    };
